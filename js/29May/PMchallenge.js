...
console.log(data)
console.log(`Sunrise is ${data.query.results.channel.astronomy.sunrise}`)
console.log(`Sunset is ${data.query.results.channel.astronomy.sunset}`)

forecastWeek = data.query.results.channel.item.forecast

console.log(forecastWeek)
forecastWeek.forEach(f => console.log(
 `On ${f.date}, the highest temperature is ${fToC(f.high)} C and lowest is ${fToC(f.low)}C and the weather is ${f.text}.`
))
...
const sunriseTime = data.query.results.channel.astronomy.sunrise
console.log(sunriseTime.split(" ")[0] + 0 + ' AM')
